                            ...
                            <li v-for="item in cartList">
                                <div class="cart-tab-1">
                                    ...
                                    <div class="cart-item-pic">
                                        <img :src="item.productImage" v-bind:alt="item.productName">
                                    </div>
...
<script>
    ...
    export default{
        ...
        methods: {
            ...
            delCartConfirm(item){//删除某一商品，先弹出模态框
                this.modalConfirm = true;
                this.delItem = item;//全局缓存，而不需要每个传参
            },
            closeModal(){//关闭模态框
                this.modalConfirm = false;
            },
            delCart(){//删除
                this.$axios.post("http://localhost:27017/users/cartDel", qs.stringify({productId: this.delItem.productId})).then((response) => {
                    let res = response.data;
                    if (res.status == "0") {
                        this.modalConfirm = false;
                        this.init();
